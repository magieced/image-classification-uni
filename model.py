...
def train_test_models():
    ...
    model3, validation3 = model_creator.train_model(epochs=30, model_number=3,create_validation_dataloader=True)
    accuracy3 = model_creator.evaluate_model(model3, validation3)
    file.write("b3 accuracy: " + str(accuracy3) + "\n")
    del(model3)
    del(validation3)

    model4, validation4 = model_creator.train_model(epochs=40, model_number=4,create_validation_dataloader=True)
